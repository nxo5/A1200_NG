...
entity cia_a_serial is
    Port (
        ...
        sp_in         : in    std_logic; -- KORREKTUR: Tastatur-Daten Lesen [14.1]
        sp_out        : out   std_logic  -- KORREKTUR: Tastatur-Daten Schreiben [14.1]
    );
end cia_a_serial;
...
begin
    ...
    begin
        if reset = '1' then
            ...
            -- 1. FLANKENERKENNUNG FÜR DEN KEYBOARD-CLOCK (cia_cnt)
            cnt_sync_r1 <= cia_cnt;
            ...
            if chip_sel = '1' and write_en = '1' and e_clock_ce = '1' then
                if reg_addr(3 downto 0) = x"C" then
                    reg_sdr <= data_in; -- $SDR beschreiben (z.B. für Sendevorgänge)
                end if;
            end if;

            -- 3. BUS-LESEZUGRIFFE DER CPU (CPU holt empfangene Tastaturdaten ab)
            if chip_sel = '1' and read_en = '1' then
                if reg_addr(3 downto 0) = x"C" then
                    reg_data_out_sync <= reg_sdr; -- $SDR auslesen
                end if;
            end if;
