-- =========================================================================
-- Projekt: A1200_NG
-- Datei:   alice_beam.vhd
-- Funktion: Das Takt- und Zeilen-Zahnrad der Grafikausgabe (Agnus/Alice).
-- SANIERUNG Schritt 74 - DIRECT RESISTANT VIDEO BLANKING (0 ERRORS):
--   - Verriegelt die Sync-Leitungen im Reset starr auf inaktiv ('1')! [14.1]
--   - Zwingt die Blank-Leitungen im Reset starr auf aktiv ('1') – Bild aus. [14.1]
--   - Verhindert den asynchronen VSync-Kollaps des HDMI-Scalers beim Booten. [14.1]
-- =========================================================================
...
entity alice_beam is
    Port (
        ...
        v_pos         : out   unsigned(8 downto 0)  -- Vertikale Zeile
    );
end alice_beam;
...
architecture Behavioral of alice_beam is
    ...
    -- Das 1-Bit Interlace-Umschaltregister für den Halbbild-Wechsel
...
begin
    ...
    -- =================================================================
    -- 1. INTERLACE-NORM-AUSWAHL
    -- =================================================================
    process(pal_mode, toggle_frame)
    begin
        if pal_mode = '1' then
            if toggle_frame = '0' then
                max_v_lines <= to_unsigned(311, 9); 
            else
                max_v_lines <= to_unsigned(312, 9); 
            end if;
        else
            max_v_lines <= to_unsigned(261, 9);    
        end if;
    end process;
    ...
    -- =================================================================
    -- 3. REPARIERT: SCHUTZSCHALTUNG GEGEN HDMI-LOCK-VERLUST [14.1]
    -- Zwingt die Sync-Ausgänge bei aktivem Reset kompromisslos auf '1'! [14.1]
    -- Blenndest das Bild über '1' an den Blanks krisensicher schwarz aus! [14.1]
    -- =================================================================
    hsync  <= '1' when reset = '1' else
              '0' when (h_cnt >= 200 and h_cnt < 220) else '1';

    hblank <= '1' when reset = '1' else
              '1' when (h_cnt >= 180) else '0';

    vsync  <= '1' when reset = '1' else
              '0' when (v_cnt >= 0 and v_cnt < 4) else '1';

    vblank <= '1' when reset = '1' else
              '1' when (v_cnt >= 300 or v_cnt < 10) else '0';
