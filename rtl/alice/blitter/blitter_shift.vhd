...
entity blitter_shift is
    Port (
        ...
        -- Die beiden neuen, lebenswichtigen Kontrolladern von der Ablauf-FSM
...
begin
    ...
    -- =================================================================
    -- 1. CPU-REGISTER- UND DMA-SCHATTEN-PROZESS (Die echte Pipeline)
    -- =================================================================
        ...
        if reset = '1' then
            ...
            if reg_write_en = '1' then
                case reg_addr is
                    ...
                    when x"040" => shift_a     <= to_integer(unsigned(reg_data_w(15 downto 12)));
                    when x"042" => shift_b     <= to_integer(unsigned(reg_data_w(15 downto 12)));
                    when others => null;
                end case;
            else
                if chan_load = "00" then
                    reg_blta_old <= reg_bltadat;
                    reg_bltadat  <= dma_data_in;
                elsif chan_load = "01" then
                    reg_bltb_old <= reg_bltbdat;
                    reg_bltbdat  <= dma_data_in;
                elsif chan_load = "10" then
                    reg_bltcdat  <= dma_data_in;
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        ...
        a_rotated  <= combined_a(15 + shift_a downto shift_a); -- Fest verankert mit downto
    end process;
    ...
    begin
        ...
        b_rotated  <= combined_b(15 + shift_b downto shift_b); -- Fest verankert mit downto
    end process;
